...
Oblivious message retrieval (OMR) allows resource-limited recipients to outsource the expensive message retrieval process in a privacy-preserving way in anonymous message delivery systems. 
This work presents a novel OMR scheme, InstantOMR, based on TFHE homomorphic encryption, specifically optimized for low latency and high parallelizability, without significantly compromising overall performance. Specifically, our implementation using Primus-fhe library (and our estimation using TFHE-rs library) demonstrates that InstantOMR has the following advantages:
- **Low latency:** InstantOMR achieves $> 480\times$ lower latency than SophOMR (existing highest throughput OMR) and $> 280\times$ lower than PerfOMR (existing lowest latency OMR) for a single message using a single CPU core, when using TFHE-rs. This also means that the recipient's waiting time in the *streaming* setting, where the detector processes new messages offline and returns a digest as soon as the recipient comes online, is also $> 480\times$ and $> 280\times$ lower than SophOMR and PerfOMR, respectively.
- **Optimal parallelizability:** With 180 physical cores, InstantOMR can achieve a ~$180\times$ speed-up compared to single-core execution. This near-optimal scalability is enabled by InstantOMR's design, which processes messages independently. Using 180 cores, InstantOMR is ~$4.5\times$ faster than SophOMR, which benefits little from multi-threading due to its reliance on BFV.
